...
type netemNodeExecClient struct {
	grpc.ClientStream
}

func (x *netemNodeExecClient) Send(m *ExecCltMsg) error {
	return x.ClientStream.SendMsg(m)
}

func (x *netemNodeExecClient) Recv() (*ExecSrvMsg, error) {
	m := new(ExecSrvMsg)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
func (c *netemClient) LinkAdd(ctx context.Context, in *LinkRequest, opts ...grpc.CallOption) (*AckResponse, error) {
	out := new(AckResponse)
	err := c.cc.Invoke(ctx, Netem_LinkAdd_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type netemNodeExecServer struct {
	grpc.ServerStream
}

func (x *netemNodeExecServer) Send(m *ExecSrvMsg) error {
	return x.ServerStream.SendMsg(m)
}

func (x *netemNodeExecServer) Recv() (*ExecCltMsg, error) {
	m := new(ExecCltMsg)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
